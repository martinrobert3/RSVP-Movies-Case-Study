...
SELECT MONTH(date_published) AS month_number, COUNT(id) AS no_of_movies 
FROM movie
...
SELECT distinct genre, count(movie_id) AS Movies_Produced_Overall
FROM genre
GROUP BY  genre
...
SELECT 
		SUM(CASE WHEN name IS NULL THEN 1 ELSE 0 END) AS name_null_val, 
		SUM(CASE WHEN height IS NULL THEN 1 ELSE 0 END) AS height_null_val,
		SUM(CASE WHEN date_of_birth IS NULL THEN 1 ELSE 0 END) AS date_of_birth_null_val,
		SUM(CASE WHEN known_for_movies IS NULL THEN 1 ELSE 0 END) AS known_for_movies_null_val
...
SELECT name AS actor_name, total_votes,
                ...
                ROUND(SUM(avg_rating*total_votes)/SUM(total_votes),2) AS actor_average_rating,
...
SELECT name AS actress_name, total_votes,
                COUNT(m.id) AS movie_count,
                ROUND(SUM(avg_rating*total_votes)/SUM(total_votes),2) AS actress_average_rating,
...
ROUND(AVG(duration) OVER (ORDER BY genre)) AS moving_average_duration
FROM movie m
INNER JOIN genre g
ON m.id = g.movie_id
GROUP BY genre;
...
SELECT production_company,
		COUNT(m.id) AS movie_count,
        ROW_NUMBER() OVER(ORDER BY count(id) DESC) AS production_comp_rank
FROM movie AS m 
INNER JOIN ratings AS r 
ON m.id=r.movie_id
...
name AS director_name, COUNT(title) AS no_of_movies, 
LEAD(date_published,1) OVER(ORDER BY date_published) AS average_inter_movie_days,
